...
export class PagosComponent implements OnInit {
  ...
  cargarSolicitudes() {
    ...
    this.http.get<any[]>('https://gestiva-pyme.onrender.com/api/admin-requests', { headers })
      .subscribe({
        next: (data) => {
  ...
  solicitarMigracion() {
    if (confirm('¿Estás seguro de que deseas solicitar la migración de tus datos y no continuar con el servicio?')) {
      ...
      this.http.post('https://gestiva-pyme.onrender.com/api/admin-requests', body, { headers })
        .subscribe({
          next: () => {
  ...
  submitPago() {
    ...
    this.http.post('https://gestiva-pyme.onrender.com/api/admin-requests', formData, { headers })
      .subscribe({
        next: () => {
